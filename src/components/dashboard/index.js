...
const apiData = {
	...
	"tickets": [
		{ "wagonNumber": 5, "carClass": "2K", "carClassName": "Купе", "ticketsRemaining": 15 },
		{ "wagonNumber": 7, "carClass": "2K", "carClassName": "Купе", "ticketsRemaining": 17 },
		{ "wagonNumber": 15, "carClass": "3П", "carClassName": "Плацкарт", "ticketsRemaining": 25 }
	],
	"predictions": [
		{ "station": "Алматы 2", "carClass": "2K", "carClassName": "Купе", "ticketsSold": 15, "count": 14 },
		{ "station": "Алматы 2", "carClass": "3П", "carClassName": "Плацкарт", "ticketsSold": 12, "count": 10 },
		{ "station": "Алматы 1", "carClass": "2K", "carClassName": "Купе", "ticketsSold": 22, "count": 22 },
		{ "station": "Караганды", "carClass": "2K", "carClassName": "Купе", "ticketsSold": 32, "count": 15 },
		{ "station": "Нур-Султан 1", "carClass": "2K", "carClassName": "Купе", "ticketsSold": 40, "count": 12 },
	]
}
const options = {
	scales: {
...
const transform = (raw, optionsSet, debug) => {
	let obj = {};
	obj.labels = raw.stations;
	let stationTicketSum = {}, stationCountSum = {};
...
const GroupedBar = () => {
	...
	useEffect(() => {
		if (!curData)
			setCurData(transform(apiData, ifilter, "initCur"));
		const url = "http://localhost:8080/api/parse?route=" + trainNumber + "&date=" + date;
		fetch(url, {
			mode: 'cors'
		}).then(r => r.json()).then(d => setInfo(d))
	...
	return <Container>
		<div className='header'>
			<h1 className='title'>Маршрут {info.from} - {info.to}, {date}</h1>
		</div>
		<div>
			<label>
				Продажи:
		...
		{[...ifilter].map(v => <div key={v}>
			<label>{v}</label>
			<input type="checkbox" name={v} checked={filter.has(v)} onChange={handleFilterChange} />
		</div>)}
	</Container>
}
